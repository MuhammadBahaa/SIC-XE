...
string line , word , start , name , locctr,temp , operand ,niFlag , xFlag , bFlag , pFlag , eFlag , B_register ;
	...
	else
	{
 		while(!file.eof())
		{
			...
			else
			{
				...
				{
					instruction >> word;
					//This word could be a label name (label  instruction), a variable name(variable BYTE/WORD ...), or a directive
						...
						else // format 3
						{
							...
							if(analysis[i][4].at(0) == '0' && analysis[i][4].at(1) == '1')
							{
								...
								else
								{
									...
									int TA = C->binToDec(C->hexToBin(d));
									int PC = 0;
									if(analysis[i+1][1] != "-")
									{
